"""Daily suggestion manager: auto-record signals, settle matured ones, report accuracy.

Subcommands:
    record      Record today's dz_hi_up signals as suggestions
    settle      Settle PENDING suggestions whose horizon has passed
    report      Show accuracy metrics (hit rate, avg return, by signal type)
"""
...
def cmd_record(settings, *, capital: float, risk_pct: float,
    ...
    # Check for duplicates - skip symbols already suggested today
    existing = set()
    ...
    for _, r in buys.iterrows():
        ...
        stop_loss = round(r["close"] * 0.93, 2)
        target = round(r["close"] * 1.05, 2)
...
def cmd_settle(settings) -> int:
    ...
    for s in pending:
        ...
        print(f"  SETTLED {sym}: net {result['actual_net_bps']}bps hit={result['hit']}")
        settled_count += 1

    summary = metadata.suggestions_summary()
    metadata.close()
    print(json.dumps({"settled_now": settled_count,
                       "skipped_insufficient_data": skipped, **summary}, indent=1))
    return 0
...
def cmd_report(settings) -> int:
    ...
    by_type = con.execute("""
        SELECT signal_type, COUNT(*), AVG(actual_return_bps), SUM(hit)*1.0/COUNT(*)
        FROM daily_suggestions WHERE status='REALIZED'
        GROUP BY signal_type ORDER BY AVG(actual_return_bps) DESC
    """).fetchall()
    con.close()
    ...
    print(json.dumps({"summary": s, "golive_gate": verdict,
                      "by_signal_type": [
                          {"type": t[0], "n": t[1],
                           "avg_net_bps": round(t[2], 1) if t[2] else None,
                           "accuracy": round(t[3], 3) if t[3] else None}
                          for t in by_type]}, indent=1))
    return 0
